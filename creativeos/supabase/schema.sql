...
-- ── Radar Trendtrack (migración 003) ─────────────────────────────────────────
-- Señales de la API de trendtrack.io publicadas por el pipeline semanal.
...
create policy tt_scaling_read on tt_scaling_ads for select to authenticated using (true);
drop policy if exists tt_snapshots_read on tt_brand_snapshots;
create policy tt_snapshots_read on tt_brand_snapshots for select to authenticated using (true);
drop policy if exists tt_suggestions_read on tt_brand_suggestions;
create policy tt_suggestions_read on tt_brand_suggestions for select to authenticated using (true);
