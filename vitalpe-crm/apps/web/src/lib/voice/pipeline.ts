import 'server-only';
/**
 * The voice pipeline, independent of how the caller authenticated.
 *
 * This module is deliberately NOT a `'use server'` file. Every export of such a
 * file becomes a callable server action, and these functions take a `Session`
 * as their first argument — exporting them that way would let a browser hand in
 * any session it liked. The `'use server'` wrappers live in
 * `lib/actions/voice.ts` (cookie) and the mobile route handlers under
 * `app/api/veu/` (bearer token); both resolve the session themselves and only
 * then call in here.
 */
...
export async function interpretTextFor(
  session: Session,
  formData: FormData,
): Promise<ProposalResult> {
  return propose(
...
export async function interpretAudioFor(
  session: Session,
  formData: FormData,
): Promise<ProposalResult> {
 ...
 */
export async function applyProposalFor(
  session: Session,
  formData: FormData,
): Promise<ApplyResult> {
...
export async function discardProposalFor(
  session: Session,
  formData: FormData,
): Promise<{ ok: boolean }> {
  const voiceNoteId = String(formData.get('voiceNoteId') ?? '');
